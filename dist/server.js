...
async function buildServer() {
    const app = (0, fastify_1.default)({
        logger: { level: env_js_1.config.nodeEnv === "development" ? "debug" : "info" },
    });
    await app.register(db_js_1.default);
    await app.register(auth_js_1.default);
    await (0, health_js_1.registerHealthRoutes)(app); // public
    await (0, auth_js_2.registerAuthRoutes)(app); // token issuance, whoami
    await (0, forecasts_js_1.registerForecastRoutes)(app); // protected
    await (0, recommendations_js_1.registerRecommendationRoutes)(app); // protected
    return app;
}
const start = async () => {
