...
type ForecastRow = {
  ...
  model_name: string;
  model_stage: "Production" | "Staging" | "None";
  generated_at: string;
};
...
export async function registerForecastRoutes(app: FastifyInstance) {
  app.get(
    ...
    async (req) => {
      ...
      const latestOnly = toBool((req.query as any).latest_only, true);

      let effectiveRunId = run_id as string | undefined;

      if (!effectiveRunId && latestOnly) {
        const runRes = await req.server.db.query<{ run_id: string }>(
          `SELECT run_id
         FROM ops.batch_run
    ...
    `;
      ...
      const { rows } = await req.server.db.query<ForecastRow>(sql, params);
